...
(*
version 0.73
- введены процедуры derivative (взятие производной) и integral (взятие неопределенного интеграла)
- добавлена интерполяция 2-го порядка (квадратичными параболами), но пользоваться ею не рекомендуется
 (она введена как раз для того, чтобы продемонстрировать ее ущербность)
